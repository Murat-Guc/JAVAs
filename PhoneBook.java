...
public class PhoneBookSolution extends ConsoleProgram {
	...
	private void runCommandLoop(HashMap<String, Integer> phoneBook) {
		while(true) {
			...
			String cmd = readLine("Enter command (printAll, add, lookup, remove): ");
	...
	// Ask the user for the name, and look up the corresponding phone #
		private void remove(HashMap<String, Integer> phoneBook) {
			String key = readLine("Key: ");
			if(!phoneBook.containsKey(key)) {
				println("Sorry but there is no key as such for " + key);
			} else {
	...
	private void addToMap(HashMap<String, Integer> phoneBook) {
		String key = readLine("Key: ");
		Integer currentNo = phoneBook.get(key);
		if (currentNo == null) {
			...
			boolean isUpdate = readBoolean ("Do you want to update the number for " + key + " ");
			if (isUpdate) {
				int phoneNumber = readInt("Phone number: " );
